...
const respondToApplication = async (req, res) => {
  ...
  try {
    ...
    const applicant = event.applicants.find(app => app.rider.toString() === riderId);
    if (!applicant) {
      return res.status(404).json({ message: "Rider did not apply for this event" });
    }
    applicant.status = action;

    // Update applied event status in User
    const appliedEvent = user.appliedEvents.find(e => e.eventId.toString() === eventId);
    if (!appliedEvent) {
      return res.status(404).json({ message: "Event not found in user's applied list" });
    }
    ...
    if (action === "accepted") {
      ...
      if (isNaN(price) || isNaN(earnings)) {
        return res.status(400).json({ message: "Invalid negotiatePrice or earnings" });
      }
